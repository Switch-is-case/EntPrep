  ...
  if (isInstalled) {
    return (
      <div className="max-w-xl mx-auto px-4 py-20 text-center">
        ...
        <h1 className="text-2xl font-bold text-slate-900 mb-2">{t("install.alreadyInstalled", lang)}</h1>
        <p className="text-slate-500 font-medium mb-8">{t("install.alreadyInstalledDesc", lang)}</p>
        <Link href="/" className="inline-block px-8 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primary-dark transition-colors">
          {t("nav.home", lang)}
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      <div className="text-center mb-12">
        <h1 className="text-3xl font-bold text-slate-900 mb-3 tracking-tight">{t("install.title", lang)}</h1>
        <p className="text-slate-600 font-medium">{t("install.desc", lang)}</p>
      </div>
      ...
      {!platform ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <button
            ...
            className="flex flex-col items-center gap-6 p-8 rounded-2xl border border-slate-200 hover:border-primary/40 hover:bg-slate-50 transition-colors group"
          >
            <AppleIcon className="w-20 h-20 text-slate-800" />
            <span className="text-lg font-bold text-slate-900">iOS (iPhone / iPad)</span>
          </button>
          <button
            onClick={() => setPlatform("android")}
            className="flex flex-col items-center gap-6 p-8 rounded-2xl border border-slate-200 hover:border-primary/40 hover:bg-slate-50 transition-colors group"
          >
            <AndroidIcon className="w-20 h-20" />
            <span className="text-lg font-bold text-slate-900">Android</span>
          </button>
        </div>
      ) : (
        <div className="space-y-8">
          ...
          <div className="space-y-3">
            {(platform === "ios" ? iosSteps : androidSteps).map((step) => (
              <div
                key={step.id}
              ...
              >
                ...
                <div className="flex-1 text-sm font-bold text-slate-700 leading-tight flex items-center gap-4">
                  <step.icon className="w-5 h-5 text-slate-400 shrink-0" aria-hidden="true" />
                  <span>{step.text}</span>
                </div>
              </div>
            ))}
