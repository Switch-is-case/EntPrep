...
export function AiExplanationBlock({
  ...
  if (!explanation && !loading && onFetch) {
    return (
      <button
        onClick={onFetch}
        className="mt-8 w-full flex items-center justify-center gap-2 px-4 py-4 rounded-2xl border-2 border-dashed border-primary/30 text-primary bg-primary/5 hover:bg-primary/10 transition-all text-sm font-bold"
      >
  ...
  return (
    <motion.div
      ...
      className="mt-8 p-5 md:p-6 rounded-2xl bg-slate-50 border border-border border-l-4 border-l-primary shadow-sm relative overflow-hidden"
    >
      ...
      {loading ? (
        ...
        <div className="text-text leading-relaxed text-sm md:text-base font-medium">
          <LatexText text={explanation || (lang === "ru" ? "Объяснение скоро появится." : "Түсіндірме жақында пайда болады.")} />
        </div>
      )}
      
      {!loading && explanation && (
        <div className="mt-4 pt-3 border-t border-slate-200/60 text-[10px] text-text-secondary flex items-center gap-1.5 font-bold uppercase tracking-wider">
